...
def profile_page(username, prof):
    ...
    league=_league_data(prof); theme=league['theme']; public_id=_public_id(prof)
    total=int(prof.get('correct_guesses',0) or 0)+int(prof.get('wrong_guesses',0) or 0)
    correct=round(int(prof.get('correct_guesses',0) or 0)*100/total,1) if total else 0
    wrong=round(int(prof.get('wrong_guesses',0) or 0)*100/total,1) if total else 0
    avatar_banned=bool(prof.get('profile_banned')); bio_banned=bool(prof.get('bio_banned'))
...
def lobby_page(username, prof=None, wallet=None):
    ...
    league=_league_data(prof); theme=league['theme']; avatar='<div class="avatar-hidden">●</div>' if prof.get('profile_banned') else _avatar_html(prof.get('avatar') or '')
...
def support_page(username, reports, active_bans=None, tickets=None, receipts=None):
    ...
    items=[]
    for r in reports:
        cat=r.get('category') or 'other'; status='حل‌شده' if r.get('status')!='open' else 'باز'; target=str(r.get('target') or '')
        media=''
        if r.get('media_mime'):
            href=f'/support/report/{int(r["id"])}/media'; mime=str(r['media_mime'])
            if mime.startswith('video/'):
                media=f'<video class="report-media-video" controls preload="metadata" src="{href}"></video>'
            ...
            else: media=f'<a class="report-media-link" target="_blank" href="{href}">👁️ مشاهده فایل گزارش</a>'
        selected=''.join(f'<option value="{v}"{" selected" if v=="username" and cat=="username" else ""}>{labels[k]}</option>' for k,v in [("all","all"),("chat","chat"),("games","games"),("drawing","drawing"),("profile","profile"),("bio","bio"),("username","username")])
        extra='<div class="username-ban-note">با تأیید محرومیت آیدی، آیدی فعلی خودکار حذف و یک آیدی موقت مثل user123456 جایگزین می‌شود؛ تا پایان محرومیت امکان تغییر آیدی ندارد.</div>' if cat=='username' else ''
        items.append(f'''<div class="report-item" data-report-id="{int(r['id'])}"><div><b>گزارش #{int(r['id'])}</b> <span class="report-category">{html.escape(cats.get(cat,cat))}</span> — {status}</div><div class="report-meta">گزارش‌دهنده: @{html.escape(str(r.get('reporter') or ''))} | کاربر: <b>@{html.escape(target)}</b></div><p>{html.escape(str(r.get('content') or ''))}</p>{media}{extra}<div class="support-action-grid"><select class="ban-scope">{selected}</select><input class="ban-hours" type="number" min="0" max="168" value="0"><input class="ban-minutes" type="number" min="1" max="59" value="10"><button class="glow-btn" onclick="banUser({int(r['id'])},this)">🚫 تأیید و محروم کن</button></div><div class="support-review-row"><button onclick="reviewReport({int(r['id'])},'valid')">✅ گزارش درست بود</button><button onclick="reviewReport({int(r['id'])},'invalid')">⚠️ گزارش بی‌اساس</button></div></div>''')
    bans=[]
    for b in active_bans:
    ...
    body=f'''<div class="card admin-card page-enter"><h1>🛡️ مرکز فرماندهی پشتیبانی</h1><p>گزارش‌ها، آیدی‌ها، محرومیت‌ها، رسیدها و لیگ‌ها.</p></div><div class="card support-admin-card"><h2>💰 مدیریت سکه و جام</h2><div class="support-direct-grid"><input id="walletTarget" placeholder="آیدی کاربر"><input id="coinDelta" type="number" placeholder="تغییر سکه"><input id="trophyDelta" type="number" placeholder="تغییر جام"><button onclick="adjustWallet()">اعمال</button></div><div id="walletStatus" class="status-line"></div></div><div class="card"><h2>🧾 رسیدها</h2>{''.join(receipt_items) or '<p>رسیدی وجود ندارد.</p>'}</div><div class="card"><h2>🔨 محرومیت مستقیم</h2><div class="support-direct-grid"><input id="directTarget" placeholder="آیدی کاربر"><select id="directScope"><option value="all">همه امکانات</option><option value="chat">چت</option><option value="games">بازی‌ها</option><option value="drawing">نقاشی</option><option value="profile">تصویر پروفایل</option><option value="bio">بیوگرافی</option><option value="username">آیدی</option></select><input id="directHours" type="number" min="0" max="168" value="0"><input id="directMinutes" type="number" min="1" max="59" value="10"></div><input id="directReason" placeholder="دلیل محرومیت"><button onclick="banDirect()">🚫 محروم کن</button></div><div class="card"><h2>⏳ محرومیت‌های فعال</h2>{''.join(bans) or '<p>محرومیت فعالی نیست.</p>'}</div><div class="card"><h2>🚩 گزارش‌ها</h2>{''.join(items) or '<p>گزارشی نیست.</p>'}</div><div class="card"><h2>🎫 تیکت‌ها</h2>{''.join(f'<div class="report-item"><b>#{int(t["id"])} — {html.escape(str(t["subject"]))}</b><p>{html.escape(str(t["content"]))}</p></div>' for t in tickets) or '<p>تیکتی نیست.</p>'}</div><div style="text-align:center"><a class="btn" href="/chat/private/morad">💬 پیوی پشتیبانی</a> <a class="btn" href="/lobby">🏠 لابی</a></div><script>
    async function post(url,payload){{const r=await fetch(url,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(payload)}});return r.json()}}
    async function doBan(p){{const d=await post('/support/ban',p);if(d.ok)location.reload();else alert(d.message||'❌ انجام نشد.')}}
    async function banUser(id,btn){{const item=btn.closest('.report-item'),target=item.querySelector('.report-meta b').textContent.trim().replace(/^@/,'');const scope=item.querySelector('.ban-scope').value;await doBan({{report_id:id,target,scope,hours:Number(item.querySelector('.ban-hours').value)||0,minutes:Number(item.querySelector('.ban-minutes').value)||0,reason:'بر اساس گزارش #'+id}})}}
    async function banDirect(){{const target=directTarget.value.trim();if(!target)return;await doBan({{target,scope:directScope.value,hours:Number(directHours.value)||0,minutes:Number(directMinutes.value)||0,reason:directReason.value.trim()||'نقض قوانین'}})}}
    async function unbanUser(btn){{const row=btn.closest('.ban-row');const d=await post('/support/unban',{{target:row.dataset.target,scope:row.dataset.scope}});if(d.ok)row.remove();else alert(d.message||'❌')}}
    ...
    async function replayReport(id){{const box=document.getElementById('replay-'+id);box.style.display='block';box.textContent='در حال دریافت بازپخش...';const r=await fetch('/support/report/'+id+'/media');const arr=await r.json();if(!Array.isArray(arr)){{box.textContent='بازپخش در دسترس نیست.';return}}const c=document.createElement('canvas');c.width=900;c.height=400;c.style.width='100%';c.style.background='#fff';box.innerHTML='';box.appendChild(c);const x=c.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,c.width,c.height);let i=0;function step(){{if(i>=arr.length)return;const s=arr[i++];x.strokeStyle=s.color||'#111';x.lineWidth=s.size||4;x.lineCap='round';x.beginPath();x.moveTo(s.x0*c.width,s.y0*c.height);x.lineTo(s.x1*c.width,s.y1*c.height);x.stroke();setTimeout(step,10)}}step()}}
    </script>'''
    return page_shell('پنل پشتیبانی',body,username)
...
def _drawing_battle_page(username: str, mode: int) -> str:
    ...
    function startRecording(){{try{{if(!window.MediaRecorder||!canvas.captureStream)return;const stream=canvas.captureStream(12);const mime=['video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm'].find(x=>MediaRecorder.isTypeSupported(x))||'';mediaRecorder=new MediaRecorder(stream,mime?{{mimeType:mime}}:undefined);mediaChunks=[];drawingBlob=null;mediaRecorder.ondataavailable=e=>{{if(e.data?.size)mediaChunks.push(e.data)}};mediaRecorder.onstop=()=>{{if(mediaChunks.length)drawingBlob=new Blob(mediaChunks,{{type:mediaChunks[0].type||'video/webm'}})}};mediaRecorder.start(1000)}}catch(e){{mediaRecorder=null}}}}
    ...
    async function confirmDrawingReport(ok){{const m=document.getElementById('drawingConfirm');if(m)m.remove();if(!ok)return;const drawer=(window.__lastDrawer||'');if(!drawer)return alert('نقاش پیدا نشد.');const fd=new FormData();fd.append('target',drawer);fd.append('content','گزارش نقاشی دور جاری');fd.append('drawing_data',JSON.stringify(currentStrokes.slice(0,6000)));if(drawingBlob)fd.append('video',drawingBlob,'drawing.webm');const r=await fetch('/report/drawing',{{method:'POST',body:fd}});const d=await r.json();alert(d.ok?'✅ گزارش برای پشتیبانی ارسال شد.':'❌ ارسال گزارش ناموفق بود.')}}
...
def chat_private_page(username, other, messages, other_display=None):
    ...
    body=f'''<div class="chat-screen page-enter"><div class="chat-top"><div class="chat-toolbar"><div class="chat-title">💬 @{html.escape(title_id)}</div><a class="btn" href="/messages">پیام‌ها</a></div></div><div class="chat-box" id="chatBox">{initial}</div><div class="chat-input-wrap"><div class="chat-input-row"><input type="text" id="msgInput" placeholder="پیامت رو بنویس..." autocomplete="off"><button class="send-btn" onclick="sendMsg()">➤</button></div></div></div><script>const wsProto=location.protocol==='https:'?'wss:':'ws:',ws=new WebSocket(wsProto+'//'+location.host+'/ws/chat/private/{html.escape(other)}'),box=document.getElementById('chatBox'),me={username!r};function addMsg(d){{const div=document.createElement('div');div.className='msg '+(d.sender===me?'me':'');const s=document.createElement('span');s.className='sender';s.textContent=d.public_username||d.sender;const c=document.createElement('span');c.className='content';c.textContent=d.content;div.append(s,c);if(d.sender!==me){{const b=document.createElement('button');b.className='report-btn';b.textContent='🚩 گزارش';b.onclick=()=>openReportModal(d.sender,d.content,'chat','');div.append(b)}}box.appendChild(div);box.scrollTop=box.scrollHeight}}ws.onmessage=e=>{{const d=JSON.parse(e.data);if(d.type==='friend_required'){{alert('💬 اول باید درخواست دوستی ارسال و پذیرفته شود.');location.href='/profile?u='+encodeURIComponent('{html.escape(other)}');return}}if(d.content!==undefined)addMsg(d)}};function sendMsg(){{const i=document.getElementById('msgInput'),v=i.value.trim();if(!v||ws.readyState!==1)return;ws.send(JSON.stringify({{content:v}}));i.value=''}}document.getElementById('msgInput').addEventListener('keydown',e=>{{if(e.key==='Enter')sendMsg()}});box.scrollTop=box.scrollHeight;</script>'''
