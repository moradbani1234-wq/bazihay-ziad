...
def _drawing_battle_page(username: str, mode: int) -> str:
    ...
    function avatarNode(profile){{const wrap=document.createElement('div');const fc={{profile_frame:'frame-premium',bronze_frame:'frame-bronze',davinci_frame:'frame-davinci',picasso_frame:'frame-picasso',gold_frame:'frame-gold',ice_frame:'frame-ice',angel_frame:'frame-angel',fire_frame:'frame-fire',lightning_frame:'frame-lightning',purple_frame:'frame-purple',king_frame:'frame-king'}}[profile?.active_frame||'']||'';wrap.className='draw-avatar '+fc;const a=profile?.avatar||'●';if(typeof a==='string'&&a.startsWith('data:image/')){{const img=document.createElement('img');img.className='draw-avatar-img';img.src=a;img.alt='';wrap.appendChild(img)}}else wrap.textContent=a||'●';return wrap}}
...
def _drawing_battle_page(username: str, mode: int) -> str:
    ...
    function avatarNode(p){{const w=document.createElement('div');w.className='draw-avatar '+frameClass(p?.active_frame);const a=p?.avatar||'';if(a.startsWith('data:image/')){{const i=document.createElement('img');i.src=a;w.appendChild(i)}}else w.textContent='●';const ex=document.createElement('i');ex.className='aura-extra';ex.setAttribute('aria-hidden','true');w.appendChild(ex);return w}}function frameClass(k){{const m={{profile_frame:'frame-premium',bronze_frame:'frame-bronze',davinci_frame:'frame-davinci',picasso_frame:'frame-picasso',gold_frame:'frame-gold',ice_frame:'frame-ice',angel_frame:'frame-angel',fire_frame:'frame-fire',lightning_frame:'frame-lightning',purple_frame:'frame-purple',king_frame:'frame-king'}};return m[k]||''}}
...
def chat_private_page(username, other, messages, other_display=None):
    ...
    function addMsg(d){{if(d.content===undefined)return;const div=document.createElement('div');div.className='msg '+(d.sender===me?'me':'')+((d.owned_items||[]).includes('chat_bubble')?' chat-bubble-owned':'');const av=document.createElement('span');av.className='chat-avatar '+frameClass(d.active_frame);if(d.avatar){{const im=document.createElement('img');im.className='chat-avatar-img';im.src=d.avatar;av.appendChild(im)}}else av.textContent='●';const ex=document.createElement('i');ex.className='aura-extra';ex.setAttribute('aria-hidden','true');av.appendChild(ex);const body=document.createElement('span');body.className='chat-message-body';const s=document.createElement('span');s.className='sender';s.textContent=d.public_username||d.sender;const c=document.createElement('span');c.className='content';c.textContent=d.content;body.append(s,c);div.append(av,body);if(d.sender!==me){{const b=document.createElement('button');b.className='report-btn';b.textContent='🚩 گزارش';b.onclick=()=>openReportModal(d.sender,d.content,'chat','');div.append(b)}}box.appendChild(div);box.scrollTop=box.scrollHeight}}function frameClass(k){{const m={{profile_frame:'frame-premium',bronze_frame:'frame-bronze',davinci_frame:'frame-davinci',picasso_frame:'frame-picasso',gold_frame:'frame-gold',ice_frame:'frame-ice',angel_frame:'frame-angel',fire_frame:'frame-fire',lightning_frame:'frame-lightning',purple_frame:'frame-purple',king_frame:'frame-king'}};return m[k]||''}}
