...
def _drawing_battle_page(username: str, mode: int) -> str:
    ...
    const proto=location.protocol==='https:'?'wss:':'ws:'; let ws=null,recon=null,intentionallyLeaving=false,timerInt=null,previewInt=null,role=null,currentColor='#20163a',currentSize=5,tool='pen',drawing=false,last=null,pending=[],polling=false,pollTimer=null,pollStarted=false;
    ...
    function sendPending(){{if(!pending.length)return;const payload={{type:'draw_batch',segments:pending}};if(polling){{fetch('/api/drawing/action/'+MODE,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(payload)}}).catch(()=>{{}});pending=[];return}}if(ws?.readyState===1){{ws.send(JSON.stringify(payload));pending=[]}}}}
    ...
    $('penTool').onclick=()=>{{tool='pen';$('penTool').classList.add('active');$('eraserTool').classList.remove('active')}};$('eraserTool').onclick=()=>{{tool='eraser';$('eraserTool').classList.add('active');$('penTool').classList.remove('active')}};$('clearBtn').onclick=()=>{{clearBoard();const p={{type:'clear'}};if(polling)fetch('/api/drawing/action/'+MODE,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(p)}}).catch(()=>{{}});else ws?.send(JSON.stringify(p))}};$('sizeSlider').oninput=e=>{{currentSize=Number(e.target.value)||5;$('sizeValue').textContent=currentSize}};
    ...
    function sendGuess(word){{const v=(word||'').trim();if(!v)return;const p={{type:'guess',word:v}};if(polling)fetch('/api/drawing/action/'+MODE,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(p)}}).catch(()=>{{}});else if(ws?.readyState===1)ws.send(JSON.stringify(p))}}
    ...
    async function startPolling(){{if(pollStarted)return;pollStarted=true;polling=true;try{{const r=await fetch('/api/drawing/join/'+MODE,{{method:'POST'}});const d=await r.json();if(!d.ok){{status.textContent='❌ ورود به بازی ممکن نشد';return}}status.textContent='🟢 حالت اتصال جایگزین فعال شد';pollEvents()}}catch(e){{status.textContent='❌ اتصال به بازی برقرار نشد'}}}}
    async function pollEvents(){{if(intentionallyLeaving)return;try{{const r=await fetch('/api/drawing/events/'+MODE,{{cache:'no-store'}});const d=await r.json();(d.events||[]).forEach(onMsg)}}catch(e){{}}pollTimer=setTimeout(pollEvents,500)}}
    function connect(){{try{{ws=new WebSocket(proto+'//'+location.host+WS_PATH);const fallback=setTimeout(()=>{{if(ws&&ws.readyState===0){{try{{ws.close()}}catch(e){{}};startPolling()}}}},2500);ws.onopen=()=>{{clearTimeout(fallback);status.textContent='🟢 وصل شد؛ در حال پیدا کردن بازیکن…'}};ws.onclose=()=>{{clearTimeout(fallback);if(!intentionallyLeaving&&!polling)startPolling()}};ws.onerror=()=>{{if(!polling)startPolling()}};ws.onmessage=e=>onMsg(JSON.parse(e.data))}}catch(e){{startPolling()}}}}
...
def _drawing_battle_page(username: str, mode: int) -> str:
    ...
    function renderResult(d){{if(timerInt)clearInterval(timerInt);stopRecording();if(battle)battle.classList.add('phase-screen');roundInfo.style.display='none';players.style.display='none';boardFrame.style.display='none';palette.style.display='none';tools.style.display='none';wordBox.style.display='none';drawerNotice.style.display='none';opts.style.display='none';reportBtn.style.display='none';preview.style.display='none';result.style.display='flex';window.__lastDrawer=d.drawer;const roundRows=Object.entries(d.points_map||{{}}).map(([u,s])=>'<div class="result-row round-points"><span>⭐</span><b>'+((d.profiles||{{}})[u]?.public_username||u)+'</b><strong>+'+s+' امتیاز</strong></div>').join('');const drawerRow=d.points_drawer?'<div class="result-row round-points"><span>🎨</span><b>'+((d.profiles||{{}})[d.drawer]?.public_username||d.drawer||'نقاش')+'</b><strong>+'+d.points_drawer+' امتیاز</strong></div>':'';result.innerHTML='<div class="result-glow">'+(d.correct?'🎯':'⌛')+'</div><div class="draw-result-title">'+(d.correct?'این دور تمام شد':'زمان این دور تمام شد')+'</div><div class="result-word">موضوع: <b>'+String(d.word||'—')+'</b></div><div class="result-subtitle">امتیاز همین دور</div><div class="result-table">'+(roundRows+drawerRow||'<div class="report-meta">این دور امتیازی ثبت نشد.</div>')+'</div><div id="breakText" class="draw-result-note"></div>';const bt=$('breakText');countdown(d.break_seconds||10,x=>bt.textContent=d.is_last?'🏁 نتیجه نهایی تا '+x+' ثانیه دیگر':'⚡ دور بعدی تا '+x+' ثانیه دیگر')}}
