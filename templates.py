...
def support_page(username: str, reports: list[dict], active_bans: list[dict] | None = None) -> str:
    ...
    <div class="card">
      ...
      <p>یک لینک مستقیم فایل صوتی HTTPS/HTTP بده. آهنگ در بالای چت عمومی برای همه نمایش داده می‌شود و هر کاربر خودش می‌تواند پخش یا قطعش کند.</p>
      <input type="text" id="musicTitle" placeholder="نام آهنگ / عنوان">
      <input type="text" id="musicUrl" placeholder="لینک مستقیم MP3 / M4A / OGG">
      <input type="text" id="musicTarget" placeholder="آیدی کاربر (اختیاری؛ برای نمایش اختصاصی)">
      <button class="glow-btn" onclick="setMusic()">🎵 فعال کردن آهنگ</button>
      <button class="btn" onclick="clearMusic()" style="margin-inline-start:6px">⏹ قطع برای همه</button>
      <div class="support-music-preview">ℹ️ به‌خاطر محدودیت مرورگر، پخش خودکار با صدا انجام نمی‌شود؛ کاربر با دکمه «پخش» شروعش می‌کند.</div>
    </div>
    ...
    async function setMusic() {{
      const title=document.getElementById('musicTitle').value.trim() || 'آهنگ چت روم';
      const url=document.getElementById('musicUrl').value.trim();
      const target_user=document.getElementById('musicTarget').value.trim();
      if(!url){{alert('لینک آهنگ را وارد کن');return;}}
      const r=await fetch('/support/music',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{title,url,target_user}})}});
      const d=await r.json();
      if(d.ok){{alert('🎵 آهنگ فعال شد و به چت روم ارسال شد.');}}else alert(d.error==='user_not_found'?'کاربر پیدا نشد.':'لینک آهنگ نامعتبر است.');
    }}
...
def _nav(username):
    if username:
        ...
        return f'''<div class="nav"><div class="nav-brand">🎲 بازی‌خونه — {u}</div><div><a href="/lobby">🏠 لابی</a><a href="/profile/{u}">👤 پروفایل</a><a href="/settings">⚙️ تنظیمات</a>{support}<a href="/logout">خروج</a></div></div>'''
...
def _render_messages(messages, username):
    ...
    for m in messages:
        ...
        sender_link=f'<a class="profile-link" href="/profile/{html.escape(sender)}">{html.escape(sender)}</a>'
